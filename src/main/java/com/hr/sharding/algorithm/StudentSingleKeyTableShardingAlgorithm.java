package com.hr.sharding.algorithm;
...
public class StudentSingleKeyTableShardingAlgorithm implements SingleKeyTableShardingAlgorithm<Integer>{  
    ...
    public String doEqualSharding(Collection<String> tableNames, ShardingValue<Integer> shardingValue) {  
        for (String each : tableNames) {  
            if (each.endsWith("0".concat(String.valueOf(shardingValue.getValue() % 1)))) {
                return each;  
            }
        }  
    ...
    public Collection<String> doInSharding(Collection<String> tableNames, ShardingValue<Integer> shardingValue) {  
        ...
        for (Integer value : shardingValue.getValues()) {  
            for (String tableName : tableNames) {  
                if (tableName.endsWith("0".concat(String.valueOf(value % 1)))) {
    ...
    public Collection<String> doBetweenSharding(Collection<String> tableNames,  
        ...
        for (Integer i = range.lowerEndpoint(); i <= range.upperEndpoint(); i++) {  
            for (String each : tableNames) {  
                if (each.endsWith("0".concat(String.valueOf(i % 1)))) {
                    result.add(each);  
                }  
            }  
        }  
